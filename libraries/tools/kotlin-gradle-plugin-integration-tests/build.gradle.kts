...
// Exposes the TestKit-produced `.exec` file to :kotlin-gradle-plugin-test-coverage. The file is
// written by the JaCoCo agent injected into TestKit child Gradle daemons (see
// testbase/testDsl.kt::addJacocoAgentIfEnabled), not by a Test task's own JVM — so we expose it
// manually instead of letting jacoco discover it from a Test task's JacocoTaskExtension.
//
// `testTask = kgpAllParallelTests` makes :kotlin-gradle-plugin-test-coverage:integrationCoverageReport
