...
class SwiftPMImportExplicitClangModulesIT : KGPBaseTest() {
    ...
    fun `discoverClangModulesImplicitly false with explicit importedClangModules exposes only the listed module`(
        ...
        project("emptyxcode", version) {
            ...
            val signatures = commonizeAndDumpCinteropSignatures().filterOutNoiseSignatures()
            assertEquals(
                """
            ...
            assertFalse(
                signatures.contains("HiddenHelper"),
                "HiddenHelper symbols must not leak into cinterop when HiddenTarget is omitted from importedClangModules"
            )
        }
    }
